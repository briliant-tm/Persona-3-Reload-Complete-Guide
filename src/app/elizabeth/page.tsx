 ...
 * Elizabeth's 36 requests across 5 categories are static game
 * data from `lib/data/elizabeth.ts`.
 *
 * Configuration:
 *   export const dynamic = 'force-static'
 *
 * Why SSG here?
 *   Quest data (names, descriptions, rewards, completion guides)
 *   is immutable game content. Category filtering, difficulty
 *   toggles, and expandable cards are client-side interactions.
 *   Pre-rendering the full set at build time ensures zero-latency
 *   page loads while keeping the interactive UX intact.
 * ============================================================
 */
...
const CATEGORY_CONFIG: Record<string, { icon: React.ReactNode; gradient: string; label: string }> = {
...
function RequestCard({ request, theme, index }: {
  request: ElizabethRequest; theme: string; index: number;
}) {
  const [expanded, setExpanded] = useState(false);
  const cat = CATEGORY_CONFIG[request.category];
  const diff = DIFFICULTY_COLORS[request.difficulty];
...
export default function ElizabethPage() {
  ...
  const filtered = useMemo(() => {
    return ELIZABETH_REQUESTS.filter((r) => {
      const matchesSearch =
  ...
  // Stats
  const totalCount = ELIZABETH_REQUESTS.length;
  const byCategory = categories.slice(1).map(c => ({
    cat: c,
    count: ELIZABETH_REQUESTS.filter(r => r.category === c).length,
    ...CATEGORY_CONFIG[c],
  }));
  ...
  return (
    <div className={`min-h-screen pt-24 pb-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden transition-colors ${
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map((r, idx) => (
          <RequestCard key={r.id} request={r} theme={theme} index={idx} />
        ))}
      </div>
