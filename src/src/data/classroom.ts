export const CLASSROOM_SECTIONS = [
  {
    title: "April",
    items: [
      { date: "4/8", question: "Among these phrases, 'a rain of flowers,' 'mystical mirage,' and 'vivid carp streamers,' which one symbolizes summer?", answer: "Vivid Carp Streamers" },
      { date: "4/18", question: "The places where people dumped their waste in the Jomon period — what are they called nowadays?", answer: "Middens" },
      { date: "4/27", question: "Leader, do you know which one’s not an algebraic spiral or whatever?", answer: "A" },
    ]
  },
  {
    title: "May",
    items: [
      { date: "5/6", question: "What do you call the device that helps generate electric power for the train?", answer: "A pantograph" },
      { date: "5/13", question: "Which tool did Leon Foucault use in his experiments regarding the rotation of the Earth?", answer: "The pendulum" },
      { date: "5/15", question: "What’s the other name for 'May sickness' — the more casual one?", answer: "May Blues" },
    ]
  },
  {
    title: "May Midterms",
    isExam: true,
    items: [
      { date: "5/19", question: "What is the other common expression used to describe 'May sickness?'", answer: "May Blues" },
      { date: "5/21", question: "Which of the following is generated by a pantograph?", answer: "Electricity" },
      { date: "5/22", question: "During which historical period were middens most commonly used?", answer: "Jomon" },
    ]
  },
  {
    title: "June",
    items: [
      { date: "6/15", question: "Which phrase means, 'Able to see things for what they really are?'", answer: "Keen Eye" },
      { date: "6/17", question: "What early religious practice was the origin of magic?", answer: "Shamanism" },
      { date: "6/22", question: "What else do people call this curve?", answer: "Witch of Agnesi" },
      { date: "6/25", question: "What’s it called when the air bubbles in a whirlpool bath hit you and make your body vibrate?", answer: "The flutter effect" },
      { date: "6/29", question: "What form of natural magic is used to find water sources?", answer: "Dowsing" },
    ]
  },
  {
    title: "July",
    items: [
      { date: "7/3", question: "“Thou shalt not recite such a tale of this world before the princess” ... What kinda tale is he talkin’ about?", answer: "About romance" },
      { date: "7/8", question: "Where is the comma splice found in the following sentence?", answer: "Between “time” and “it’s”" },
      { date: "7/9", question: "At the same time, a serious problem arose for the farmers. Do you remember what that was?", answer: "Social disparity" },
      { date: "7/10", question: "What is the esoteric study of Jewish texts called?", answer: "Kabbalah" },
      { date: "7/11", question: "What do you think that item (essential to battle) was?", answer: "The Katana" },
    ]
  },
  {
    title: "July Finals",
    isExam: true,
    items: [
      { date: "7/14", question: "Which form of magic is used to find water sources?", answer: "Dowsing" },
      { date: "7/15", question: "How did the Witch of Agnesi gain the first half of its name?", answer: "An error in translation" },
      { date: "7/16", question: "Who designed the prototype for the katana?", answer: "Taira No Masakado" },
      { date: "7/17", question: "There is an error in this sentence. What kind is it?", answer: "Comma splice" },
    ]
  },
  {
    title: "September",
    items: [
      { date: "9/1", question: "Which property of electricity is relevant to superconductivity?", answer: "Resistance" },
      { date: "9/10", question: "H-hey, leader, do you know what it means to 'spill the beans?'", answer: "To reveal a secret" },
      { date: "9/11", question: "Which book helped further the art of magic during the Renaissance?", answer: "The Hermetica" },
      { date: "9/14", question: "Who is one referring to when speaking of their 'better half?'", answer: "Their soulmate" },
      { date: "9/26", question: "What is the collective name for the four holy numbers in numerology?", answer: "Tetractys" },
    ]
  },
  {
    title: "October",
    items: [
      { date: "10/7", question: "Which hormone is the one in your brain that provides a sense of achievement?", answer: "Dopamine" },
      { date: "10/10", question: "Who was the founder of Theosophy, a group that gave rise to many other magical societies?", answer: "Helena Blavatsky" },
    ]
  },
  {
    title: "October Midterms",
    isExam: true,
    items: [
      { date: "10/13", question: "The hormone dopamine is related to our sense of accomplishment. Name a symptom of its over-production.", answer: "Addiction" },
      { date: "10/14", question: "Who is considered by many to be the first numerologist, whose name lives on today in a mathematical theorem?", answer: "Pythagoras" },
      { date: "10/15", question: "What happens when electrical resistance reaches zero?", answer: "Superconductivity" },
      { date: "10/16", question: "What is the meaning of the phrase, 'To spill the beans?'", answer: "To reveal a secret" },
    ]
  },
  {
    title: "October (Continued)",
    items: [
      { date: "10/19", question: "We used the number 'zero' a lot. Do you know where it originally came from, though?", answer: "India" },
      { date: "10/22", question: "Which Roman goddess has been theorized as the namesake for the month of April?", answer: "Venus" },
      { date: "10/26", question: "But there’s one region that instead calls it 'the month with gods,' because that’s where they all go! Do you know which one it is?", answer: "Izumo" },
      { date: "10/30", question: "A certain enzyme brings out the sweetness in [sweet potatoes] when they’re baked. Do you happen to know what it is?", answer: "Beta-amylase" },
    ]
  },
  {
    title: "November",
    items: [
      { date: "11/7", question: "What is the ancient Indian magical text I mentioned today?", answer: "The Upanishads" },
      { date: "11/12", question: "“Winter, early mornings.” What was she describing with such a sparse line?", answer: "Her favorite time in winter" },
      { date: "11/30", question: "In the poem that Genji, the protagonist, sends to [Murasaki-no-Ue], what did he compare her to?", answer: "The cherry blossom" },
    ]
  },
  {
    title: "December",
    items: [
      { date: "12/7", question: "What’s the name of the stratospheric layer of molecules comprised of three oxygen atoms?", answer: "The ozone layer" },
      { date: "12/9", question: "What sorcery was Himiko said to have used?", answer: "Kido" },
      { date: "12/11", question: "Do you know which of the following is an irregular plural noun?", answer: "Lives" },
    ]
  },
  {
    title: "December Finals",
    isExam: true,
    items: [
      { date: "12/14", question: "Which of the following originated in India?", answer: "The number zero" },
      { date: "12/15", question: "When heated, the beta-amylase enzymes in sweet potatoes react with the starch present to produce which disaccharide?", answer: "Maltose" },
      { date: "12/16", question: "Which Roman goddess has been theorized as the namesake for the month of April?", answer: "Venus" },
      { date: "12/17", question: "In The Tale of Genji, who does Hikaru Genji compare to cherry blossoms?", answer: "Murasaki-no-Ue" },
      { date: "12/18", question: "Which of the following words is not in the correct plural form?", answer: "Geeses" },
    ]
  },
  {
    title: "December (Continued)",
    items: [
       { date: "12/22", question: "What do we call the feeling of joy that comes from a near-death experience?", answer: "Euphoria" },
    ]
  },
  {
    title: "January",
    items: [
      { date: "1/8", question: "“The new year is the index of a journey to a certain place.” Where is this “certain place” he’s referring to?", answer: "The underworld" },
      { date: "1/18", question: "I’ve mentioned two Greek sorceresses. One was Medea. Who was the other?", answer: "Circe" }
    ]
  }
];
